...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
    ...
    /**
     * 表示从哪一时刻开始消费，格式为 yyyyMMDDHHmmss，默认为半小时前。当 consumeFromWhere=CONSUME_FROM_TIMESTAMP 时，consumeTimestamp设置的值才生效。
    ...
    /**
     * 一个Topic最大能缓存的消息数。超过该值则采取拉取流控措施。该字段默认值是-1，该值根据 pullThresholdForQueue 的配置决定是否生效，pullThresholdForTopic的优先级低于pullThresholdForQueue。
