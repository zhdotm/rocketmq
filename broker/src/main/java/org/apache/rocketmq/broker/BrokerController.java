...
public class BrokerController {
    ...
    private void initialTransaction() {
        //事务消息主要用于处理服务，默认实现类是TransactionalMessageServiceImpl。
        // 如果想自定义事务消息处理实现类，需要实现 TransactionalMessageService 接口，然后通过ServiceProvider.loadClass（）方法进行加载。
        this.transactionalMessageService = ServiceProvider.loadClass(ServiceProvider.TRANSACTION_SERVICE_ID, TransactionalMessageService.class);
        if (null == this.transactionalMessageService) {
            this.transactionalMessageService = new TransactionalMessageServiceImpl(new TransactionalMessageBridge(this, this.getMessageStore()));
            log.warn("Load default transaction message hook service: {}", TransactionalMessageServiceImpl.class.getSimpleName());
        }
        //事务消息回查监听器，默认实现类是DefaultTransactionalMessageCheckListener。
        // 如果想自定义回查监听处理，需要继承AbstractTransactionalMessageCheckListener接口，然后通过ServiceProvider.loadClass（）方法被加载。
